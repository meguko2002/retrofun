    ...
    with Session() as session:
        with session.begin():
            with open('orders.csv') as f:
                ...
                for row in reader:                                       
                    if row['name'] not in all_customers:  # customerの名前を探している
                    ...
                    product = all_products.get(row['product1'])  # productオブジェクトを探している
                    if product is None:
                        product = session.scalar(select(Product).where(Product.name==row['product1']))
                        all_products[row['product1']] = product
                    o.order_items.append(OrderItem(product=product, unit_price=float(row['unit_price1']),
                                                   quantity=int(row['quantity1'])))        
                    
                    if row['product2']:
                        product = all_products.get(row['product2'])  # product1の操作と同じ
                        if product is None:
                            product = session.scalar(select(Product).where(Product.name==row['product2']))
                            all_products[row['product2']] = product
                        o.order_items.append(OrderItem(product=product, unit_price=float(row['unit_price2']),
                    ...
                    if row['product3']:
                        ...
                        o.order_items.append(OrderItem(product=product, unit_price=float(row['unit_price3']),
